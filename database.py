...
PLANS = {
    ...
    'ultra': {'credits': -1,   'bonus': 0,    'label': 'Ultra'},  # -1 = unlimited
}
...
class User(db.Model):
    ...
    credits          = db.Column(db.Integer, default=50)   # regular credits
    bonus_credits    = db.Column(db.Integer, default=300)  # bonus credits (used first)
    ...
    def total_credits(self):
        if self.plan == 'ultra': return -1  # unlimited
        return (self.bonus_credits or 0) + (self.credits or 0)

    def deduct_credit(self):
        """Deduct 1 credit — bonus first, then regular"""
...
class ChatSession(db.Model):
    """One chat session per user (their conversation with AI / operator)"""
    __tablename__ = 'chat_sessions'

    id           = db.Column(db.Integer, primary_key=True)
    user_id      = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    session_key  = db.Column(db.String(64), unique=True, nullable=False)  # random key for polling
    operator_on  = db.Column(db.Boolean, default=False)   # True = human operator took over
    operator_id  = db.Column(db.Integer, nullable=True)   # which operator is handling
    created_at   = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at   = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    user         = db.relationship('User', backref='chat_sessions', foreign_keys=[user_id])
